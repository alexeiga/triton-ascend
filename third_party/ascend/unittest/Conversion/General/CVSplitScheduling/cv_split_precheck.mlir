...
// RUN: triton-opt %s "--cv_split_scheduling=compile-on-910-95=true unroll-factor=2" 2>&1 >/dev/null | FileCheck %s --check-prefix=FACTOR2
// RUN: triton-opt %s "--cv_split_scheduling=compile-on-910-95=true unroll-factor=8" 2>&1 >/dev/null | FileCheck %s --check-prefix=FACTOR8
