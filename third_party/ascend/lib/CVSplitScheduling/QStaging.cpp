...
static FailureOr<QStagingCandidates>
collectQStagingCandidates(func::FuncOp funcOp) {
  ...
  if (candidates.empty()) {
    llvm::errs() << "[cv-split] No Q staging candidate found\n";
    return failure();
  }
  return candidates;
}

static FailureOr<QSourceChain> findQSourceChain(Value q,
                                                func::FuncOp funcOp) {
  ...
  for (Operation *user : qMem.getUsers()) {
    ...
    if (fillCopy) {
      funcOp.emitError(
          "multiple copies initialize a loop-invariant matmul LHS");
      return failure();
    }
    fillCopy = copy;
  }
  ...
  if (!fillCopy) {
    funcOp.emitError(
        "failed to find the GM source for a loop-invariant matmul LHS");
    return failure();
  }
  ...
  if (!allocation) {
    funcOp.emitError(
        "loop-invariant matmul LHS is not backed by memref.alloc");
    return failure();
  }
...
static memref::AllocOp createPersistentQBuffer(OpBuilder &builder, Location loc,
                                               RankedTensorType tt,
  ...
  auto cbufAddressSpace =
      builder.getAttr<hivm::AddressSpaceAttr>(hivm::AddressSpace::L1);
  auto cbufType = MemRefType::get(tt.getShape(), tt.getElementType(), nullptr,
                                 cbufAddressSpace);
  auto cbufAlloc = builder.create<memref::AllocOp>(loc, cbufType);

  auto uniqueMark =
      builder.create<annotation::MarkOp>(loc, cbufAlloc.getResult());
  uniqueMark->setAttr("mem_unique", builder.getUnitAttr());

  auto effectsMark =
      builder.create<annotation::MarkOp>(loc, cbufAlloc.getResult());
  effectsMark->setAttr(
      "effects", builder.getArrayAttr({builder.getStringAttr("write"),
                                        builder.getStringAttr("read")}));
  return cbufAlloc;
}

static bufferization::ToTensorOp
stageQIntoCbuf(OpBuilder &builder, Location loc, RankedTensorType tt,
               Block *scopeBody, Value srcMemref,
               memref::AllocOp cbufAlloc) {
  ...
  builder.create<annotation::MarkOp>(
      loc, qBindTensor.getResult(), ValueRange{cbufAlloc.getResult()},
...
static void replaceQMatmulLhsUses(Value q,
                                  bufferization::ToTensorOp qReadTensor,
                                  scope::ScopeOp cubeScope) {
  q.replaceUsesWithIf(qReadTensor.getResult(), [&](OpOperand &use) {
    Operation *owner = use.getOwner();
    return cubeScope->isProperAncestor(owner) &&
           isa<linalg::MatmulOp, linalg::MatmulTransposeBOp>(owner) &&
...
LogicalResult bindLoopInvariantMatmulLhsToCbuf(func::FuncOp funcOp) {
  ...
  FailureOr<QStagingCandidates> lhsToScope =
      collectQStagingCandidates(funcOp);
  ...
  for (auto &kv : *lhsToScope) {
    ...
    auto qReadTensor =
        stageQIntoCbuf(builder, loc, tt, scopeBody, srcMemref, cbufAlloc);
